...
SH_API void
sh_cmp_byte_r_v (struct _sharna_vm_s *vm, char r1, char v)
{
  vm->cpu.reg_8[R_SF] &= ~(REG_SF_ZF | REG_SF_SF | REG_SF_CF | REG_SF_OF
                           | REG_SF_AF | REG_SF_PF);

  uint8_t r = vm->ram.v[vm->cpu.reg_8[r1]] - v;
  ...
  uint8_t is_zero = (r == 0);
  uint8_t is_negative = ((r >> 7) & 1);
  uint8_t has_carry = (rv1 < v1);

  // printf ("%d %d %d\n", is_zero, is_negative, has_carry);
...
SH_API void
sh_cmp_byte_r_r (struct _sharna_vm_s *vm, char r1, char r2)
{
  vm->cpu.reg_8[R_SF] &= ~(REG_SF_ZF | REG_SF_SF | REG_SF_CF | REG_SF_OF
                           | REG_SF_AF | REG_SF_PF);
